...
import { RoutedTabsMod } from './RoutedTabsMod';
...
describe('RoutedTabs', () => {
  it('renders simplest case', async () => {
    const rendered = await renderInTestApp(
      <RoutedTabsMod routes={[testRoute1]} />,
    );
  ...
  it('navigates when user clicks different tab', async () => {
    const rendered = await renderInTestApp(
      <Routes>
        <Route
          path="/*"
          element={<RoutedTabsMod routes={[testRoute1, testRoute2, testRoute3]} />}
        />
      </Routes>,
    );
  ...
  it('shows only one tab contents at a time', async () => {
    const rendered = await renderInTestApp(
      <RoutedTabsMod routes={[testRoute1, testRoute2]} />,
